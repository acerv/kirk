...
class TestScheduler(Scheduler):
    ...
    async def _stop_tests(self, wait_schedule: bool = True) -> None:
        """
        Inner method to gracefully stop current tests.
        """
        self._logger.info("Stopping tests execution")

        # setting stop flag will prevent scheduler to add any test
        ...
        try:
            ...
            async with self._running_tests_sem:
                if wait_schedule:
                    # wait to complete the current tests scheduling
    ...
    async def stop(self) -> None:
        await self._stop_tests(wait_schedule=True)
    ...
    async def schedule(self, jobs: list) -> None:
        ...
        async with self._schedule_lock:
            ...
            try:
                ...
                await self._stop_tests(wait_schedule=False)
